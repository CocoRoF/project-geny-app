...
| **퀵챗** | 전역 단축키로 다른 앱 위에 떠서 바로 묻습니다. 한 줄로 열려 답변 길이에 맞춰 자랍니다 |
| **컴퓨터 조작** | 에이전트가 초점 있는 창에 실제로 입력·클릭합니다. 기본 꺼짐 · 권한별 · 작업마다 확인. **네이티브 모듈 0개** |
| **단축키** | 전역 단축키를 앱 안에서 다시 지정. 다른 앱이 점유하면 그렇다고 표시 |
| **자동 업데이트** | 켜짐이 기본 — 6시간마다 확인·다운로드·재시작 안내. 끄면 알림만. AppImage · deb · Windows |
| **시작·트레이** | 로그인 시 자동 시작(트레이로 조용히) · 트레이에서 컴퓨터 조작 차단 · 업데이트 |
| **로그** | 앱이 한 일을 앱 안에서 봅니다 — 엔진 상태 · 단축키 충돌 · 업데이트, 복사 가능 |
...
| `node test/app-shell.mjs` | 단일 인스턴스 · 단축키 재지정 · 자동 시작 · 업데이트 설정 · 로그 · 설정 UI 노출 |
| `node test/computer-use.mjs` | 실제 키 입력이 초점 창에 도달 · 권한 게이트 · 스크린샷 좌표 보정 |
| `node test/mcp-probe.mjs` | MCP 서버를 저장 전에 띄워 확인 (실서버 핸드셰이크 + 실패 3종) |
