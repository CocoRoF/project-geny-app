...
> **이 문서는 v0.1.0 시점의 기록입니다.** 여기서 지적한 항목은 이후 전부 해소됐고,
> 아래 표가 어디서 해소됐는지를 가리킵니다. 현재 상태를 알고 싶다면
> [`README.md`](../README.md) 와 [`ARCHITECTURE.md`](./ARCHITECTURE.md) 를 보세요.
>
> | v0.1.0 지적 | 해소 |
> |---|---|
> | Cron 이 저장만 되고 발화 안 됨 | `43f7222` — `CronRunner`+`BackgroundTaskRunner(registry, executors)` 배선 |
> | 도구 36종 중 10종만 노출 | `ec80a45` — 에이전트별 도구 선택이 매니페스트 재빌드로 실제 반영 (현재 41종) |
> | 승인 요청이 사용자에게 안 감 | `1ee935e` — `AppRequester` 를 HITL 슬롯에 설치 (`hitl requester = app_dialog`) |
> | connector 급 데스크톱 기능 0 | `43f7222`(호스트 도구 브리지) → 브라우저·퀵챗·지식·기억·훅·아바타·음성 |
> | 프로토콜에 엔진→앱 방향이 없음 | `host_tool_call`/`host_tool_result` 추가 |
