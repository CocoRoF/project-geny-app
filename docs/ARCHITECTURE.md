...
## 결정 10 — 음성은 서빙하지 않고 호출한다

**선택**: 앱은 STT/TTS 를 **클라이언트로만** 다룬다. 제공자는 STT 3종(Geny whisper-stt ·
OpenAI · 직접 지정), TTS 4종(Geny omnivoice · OpenAI · OS 내장 · 직접 지정).

**왜**: 오디오 자체 서빙은 범위에서 제외됐고(사용자 지시), 실제로 Geny 의 음성 계층은 이미
`geny-audio-services` 라는 **독립 컨테이너**다. 앱이 할 일은 그것을 *제대로 부르는 것*이다.

**"제대로"가 실제로 의미하는 것** — 계약을 읽지 않으면 조용히 틀리는 것들:
- omnivoice 의 `mode` 기본값은 `auto` = **매 호출마다 임의의 목소리**. 프로필을 고른다는 것은
  `GET /voices` → `ref_audios[emotion]` → `mode:"clone"` + `ref_audio_path` + `ref_text` 로
  **해석하는 일**이다. 그냥 `{text}` 만 보내면 목소리가 매번 바뀐다.
  (서비스 README 예제의 `voice_profile`/`format` 은 `TTSRequest` 의 필드가 아니다. 스키마가 계약이다.)
- omnivoice `/health` 는 모델 로딩 중에도 **200** 을 준다. `status:"loading"` 을 보지 않으면
  "연결됨"이라고 표시해 놓고 첫 요청에서 503 이 난다.
- whisper-stt 는 vLLM 의 OpenAI 호환 서버라 `model` 필드가 **필수**다(모델이 하나뿐이어도).
- 따라서 `openai` 와 `whisper` STT 는 **하나의 구현**이다. 두 프로토콜인 척하면 고칠 곳이 둘이 된다.

**재생 위치**: 오디오는 정확히 한 창에서만 재생한다(아바타가 떠 있으면 아바타, 아니면 본 창).
두 창에 보내면 사용자에게 두 번 들린다. 아바타가 받는 이유는 **실제 파형**으로 입을 움직이기
위해서다 — 토큰 스트림에 맞춘 가짜 리듬이 아니라.
...
않는다.** 폴더에 이미 있는 것을 표시할 줄 알 뿐이고, 없으면 *무엇이* 없는지 정확히 말한다
(설정 패널에도, 생성된 페이지 자체에도).

생성된 `index.html` 은 **다시 덮어쓰지 않는다** — 페이지가 생긴 뒤로는 사용자의 페이지가
그 아바타의 표현 방식에 대한 최종 권한이다.
...
## 결정 11 — 커넥터의 데스크톱 기능을, 네이티브 모듈 없이

geny-connector(0.22.2, 10,216줄)를 전수 대조해 이 앱에 없던 것을 채웠다.
설계상 하나만은 그대로 가져올 수 없었다.

**컴퓨터 조작**: 커넥터는 `@nut-tree-fork/nut-js` — 네이티브 애드온이다. Electron 버전마다
ABI 재빌드, 플랫폼마다 prebuild가 필요하다. 이 앱의 "받아서 실행하면 끝"은 **네이티브 모듈 0개**
위에 서 있으므로, 대신 각 OS가 이미 가진 도구로 보낸다.

| | 도구 | 동봉 여부 |
|---|---|---|
| macOS | `osascript` (System Events) | OS 기본 |
| Windows | PowerShell (SendKeys · user32) | OS 기본 |
| Linux | `xdotool`(X11) · `ydotool`(Wayland) | **없을 수 있음** → deb `recommends`, 없으면 이름을 대고 거부 |

모든 명령은 `execFile` + 인자 배열로 나간다. 타이핑할 텍스트는 모델 출력이고, 셸을 거치면
그건 문법이 된다.

**게이트가 기능만큼 중요하다.** 워크스페이스 밖에서 사용자로서 행동하는 유일한 능력이므로:
기본 꺼짐 · 권한별(입력/앱/클립보드) · 기본은 작업마다 확인 · "이번 실행 동안 허용" ·
트레이에서 즉시 차단. 확인 창은 *무엇을 입력할지*를 보여준다 — "입력 허용"에 동의하는 것은
동의가 아니다.

**좌표는 스크린샷 픽셀 공간이다.** 모델은 자기가 본 곳을 클릭한다. 캡처가 축소됐거나 HiDPI면
그대로 클릭하면 엉뚱한 곳이 눌린다. 두 공간이 같은 화면을 덮으므로 비율 보정이 전부이고,
그것으로 어떤 DPI·어떤 캡처 크기에서도 맞는다.

## 결정 12 — 단일 인스턴스는 데이터 루트 단위

Electron의 락은 `userData` 에 산다. `GENY_DATA_ROOT` 가 지정되면 `userData` 를 그 아래로
옮겨, 규칙이 "머신당 하나"가 아니라 **"데이터 루트당 하나"** 가 된다. 막아야 할 실패는
하나의 SQLite·하나의 워크스페이스를 두 프로세스가 쓰는 것이지, 개발 실행과 패키지 실행이
공존하는 것이 아니다. (그리고 테스트가 서로 다른 임시 루트로 병렬 실행될 수 있게 된다.)

## 결정 13 — 자동 업데이트는 기본 켜짐, deb 포함

기억해서 받아야 하는 업데이트는 대부분 받지 않는다. 그래서 기본은 켜짐이고 무인이며,
모든 경로가 스피너가 아니라 확정된 상태로 끝난다.

- 켜짐 → 기동 20초 후 + 6시간마다 확인·다운로드·재시작 제안
- 꺼짐 → **확인은 계속**하고 알림만 (업데이트를 원치 않는 것이지, 모르고 싶은 것이 아니다)
- 수동 → 한 번 누르면 확인·다운로드·설치·재시작까지 간다. 누른 것이 이미 결정이다

**deb 도 자동 업데이트된다.** electron-updater 6 의 `DebUpdater` 가 `dpkg -i` 를 권한 승격과
함께 실행한다. v0.2.0 에서 "deb 는 자동 업데이트 불가"라고 적었던 것은 틀렸다. 다만 Linux
패키지 설치는 Electron 의 relauncher 를 거치면 새 프로세스에 NoNewPrivs 가 넘어가고, SUID
`chrome-sandbox` 가 승격하지 못해 Ubuntu 24.04 에서 SIGTRAP 으로 죽는다 — 그래서
force-run 없이 설치하고 NNP=0 인 현재 프로세스에서 직접 되살린다.

## 결정 14 — 이식하지 않은 것

- **sync / drive** — 클라우드 기능은 범위에서 제외(사용자 지시). 커넥터 코드의 약 20%가 여기다.
- **serverConfig / 로그인** — 서버가 없다.
- **winauto (Windows UI 자동화)** — COM 기반 네이티브 표면. 결정 11 과 같은 이유로 보류.
- **i18n** — 커넥터는 ko/en. 1인용 앱에 대한 가치가 낮아 한국어 단일로 둔다.
